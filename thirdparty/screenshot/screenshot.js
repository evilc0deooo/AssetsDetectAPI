function screenshot(url, save_name) {
    ...
    // 修复错误 "cannot access member evaluate' of deleted QObject"
    function pageRender() {
        ...
        // Introduce a short delay (e.g., 1 second) to allow page.evaluate to complete
        setTimeout(function () {
...
function main() {
    ...
    if (typeof (url) === 'undefined' || url.length == 0 || typeof (save_name) === 'undefined' || save_name.length == 0) {
        console.log("Usage: phantomjs_arm screenshot.js -u=http://www.baidu.com/ -s=baidu.jpg ");
